...
def parse_arguments():
    ...
    parser.add_argument('--valid-freq',type=int, default=1,help="Frequency of validation")
    ...
    parser.add_argument('--aws-or-psc', default='psc', type=str)
    parser.add_argument('--net', default='vgg', type=str, help='alexnet or vgg')
...
def train(train_loader, net, criterion, optimizer, epoch, logger):
    ...
    for i, data in enumerate(train_loader):
        ...
        if (i + 1) % args.print_freq == 0:
            ...
            print(outputs[:,:,55:60,55:60])
            print('[epoch %d], [iter %d / %d], [train loss %.5f], [time taken %f]' % (
                epoch, i + 1, len(train_loader), train_loss.avg, time.time()-start
            ))
            start=time.time()
...
def validate(val_loader, net, criterion, optimizer, epoch, logger, ckpt_path, exp_name):
    ...
    if mean_iu > train_args['best_record']['mean_iu']:
        ...
        folder = os.path.join(ckpt_path, exp_name)
